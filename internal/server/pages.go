...
.pwd-wrap{position:relative}.pwd-wrap input{padding-right:36px!important}.pwd-toggle{position:absolute;right:8px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:1rem;color:var(--dim);padding:2px 4px;line-height:1;user-select:none}.pwd-toggle:hover{color:var(--text)}
@media(max-width:768px){.dashboard-grid{grid-template-columns:1fr}.sidebar{position:static}}
@media(max-width:640px){.navbar{padding:0 .5rem;height:48px}.navbar .logo{font-size:.85rem}.navbar .nav-right .email{display:none}.navbar .btn-icon span{display:none}.navbar .btn-icon{padding:4px 6px;font-size:.85rem}.lang-toggle a{padding:2px 5px;font-size:.7rem}.container{padding:0 .75rem;margin-top:1rem}.auth-card{padding:1.5rem}.auth-card h1{font-size:1.1rem}.auth-field input{padding:9px 10px;font-size:.85rem}.captcha-row .captcha-img{width:120px;height:38px}.site-item .site-head{flex-direction:column;align-items:flex-start;gap:.5rem}.site-item .site-head .actions{width:100%;justify-content:flex-start;flex-wrap:wrap}.site-item .site-head .name{font-size:.8rem;flex-wrap:wrap}.site-item .site-detail .detail-row{flex-direction:column;gap:.1rem}.site-item .site-detail .detail-row .label{min-width:auto}.modal{max-width:calc(100vw - 2rem);padding:1.2rem}.toast{bottom:1rem;right:1rem;left:1rem;text-align:center}}
@media(prefers-reduced-motion:reduce){*{transition:none!important}}
</style>
<script>var _t="light";try{_t=localStorage.getItem("theme")||"light"}catch(e){}document.documentElement.setAttribute("data-theme",_t)</script>
</head>
...
function togglePwd(btn){var inp=btn.parentElement.querySelector("input");if(!inp)return;var show=inp.type==="password";inp.type=show?"text":"password";btn.textContent=show?"🙈":"👁"}
...
function showLogin(){
document.getElementById("auth-form").innerHTML='<form onsubmit="doLogin();return false"><div class="auth-field"><label>'+t("email")+'</label><input id="email" type="email" placeholder="'+t("emailPh")+'" autocomplete="email"></div><div class="auth-field"><label>'+t("password")+'</label><div class="pwd-wrap"><input id="password" type="password" placeholder="'+t("password")+'" autocomplete="current-password"><button type="button" class="pwd-toggle" onclick="togglePwd(this)">👁</button></div></div><div class="auth-field"><label class="captcha-label">'+t("captchaLabel")+'</label><div class="captcha-row"><div class="captcha-img" id="login-captcha-img" onclick="loadCaptcha(\'login\')" title="'+t("captcha")+'"></div><input id="login-captcha" type="text" placeholder="'+t("captchaPh")+'"></div></div><button class="btn btn-primary" type="submit">'+t("login")+'</button></form>'+(regOpen?'<p class="switch">'+t("noAccount")+' <a onclick="showRegister()">'+t("register")+'</a></p>':'');
loadCaptcha("login");
}
function showRegister(){
document.getElementById("auth-form").innerHTML='<form onsubmit="doRegister();return false"><div class="auth-field"><label>'+t("email")+'</label><input id="email" type="email" placeholder="'+t("emailPh")+'" autocomplete="email"></div><div class="auth-field"><label>'+t("password")+'</label><div class="pwd-wrap"><input id="password" type="password" placeholder="'+t("password")+'" autocomplete="new-password"><button type="button" class="pwd-toggle" onclick="togglePwd(this)">👁</button></div><div class="field-hint">'+t("pwdHint")+'</div></div><div class="auth-field"><label>'+t("confirmPassword")+'</label><div class="pwd-wrap"><input id="confirm-pwd" type="password" placeholder="'+t("confirmPassword")+'" autocomplete="new-password"><button type="button" class="pwd-toggle" onclick="togglePwd(this)">👁</button></div></div><div class="auth-field"><label class="captcha-label">'+t("captchaLabel")+'</label><div class="captcha-row"><div class="captcha-img" id="register-captcha-img" onclick="loadCaptcha(\'register\')" title="'+t("captcha")+'"></div><input id="register-captcha" type="text" placeholder="'+t("captchaPh")+'"></div></div><button class="btn btn-primary" type="submit">'+t("register")+'</button></form><p class="switch">'+t("haveAccount")+' <a onclick="showLogin()">'+t("login")+'</a></p>';
...
h+='<li class="site-item"><div class="site-head" onclick="toggleDetail('+s.id+')"><div class="info"><div class="name"><span class="status-dot '+dot+'"></span>'+esc(s.name)+' '+badge+' '+vis+'</div><div class="url">~/sites/'+esc(s.slug)+'/</div></div><div class="actions"><a class="btn btn-sm btn-outline" href="'+siteUrl(s.url)+'" target="_blank" onclick="event.stopPropagation()">'+t("visit")+'</a><label class="upload-btn" id="upload-btn-'+s.id+'" onclick="event.stopPropagation()">'+t("deployBtn")+'<input type="file" onchange="deploy('+s.id+',this.files[0])"></label><button class="btn btn-sm btn-danger" onclick="event.stopPropagation();delSite('+s.id+')">'+t("delete")+'</button></div></div><div class="site-detail" id="detail-'+s.id+'"><div class="detail-row"><span class="label">'+t("accessPassword")+'</span><span class="value">'+pwd+'</span></div><div class="detail-row"><span class="label">'+t("visits")+'</span><span class="value" style="display:flex;gap:1rem;font-family:var(--mono);font-size:.75rem"><span>'+t("visitsToday")+': <b style="color:var(--accent)">'+vs.today+'</b></span><span>'+t("visitsMonth")+': <b style="color:var(--accent)">'+vs.month+'</b></span><span>'+t("visitsTotal")+': <b style="color:var(--accent)">'+vs.total+'</b></span></span></div></div></li>'}
...
try{var data=JSON.parse(xhr.responseText);if(xhr.status>=200&&xhr.status<300){toast(t("deployed"));loadSites()}else{toast(data.error||"request failed","error")}}catch(e){toast("network error","error")}
};
xhr.onerror=function(){if(btn){btn.firstChild.textContent=origText;btn.style.opacity="";btn.style.pointerEvents=""}toast("network error","error")};
xhr.send(fd);
}
function delSite(id){
...
.modal .modal-actions{display:flex;gap:.5rem;justify-content:flex-end;margin-top:1rem}
.pwd-wrap{position:relative}.pwd-wrap input{padding-right:36px!important}.pwd-toggle{position:absolute;right:8px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:1rem;color:var(--dim);padding:2px 4px;line-height:1;user-select:none}.pwd-toggle:hover{color:var(--text)}
@media(max-width:640px){.navbar{padding:0 .5rem;height:48px}.navbar .logo{font-size:.85rem}.stats-grid{grid-template-columns:1fr}.lang-toggle a{padding:2px 5px;font-size:.7rem}.admin-layout{grid-template-columns:1fr!important}.admin-sidebar{position:static!important;flex-direction:row;overflow-x:auto;height:auto!important;border-right:none;border-bottom:1px solid var(--line);padding:.5rem .5rem}.admin-nav-item{white-space:nowrap;padding:.4rem .6rem;font-size:.75rem}.admin-nav-item span{display:none}.admin-main{padding:.75rem!important}.container{padding:0 .75rem;margin-top:1rem}.modal{max-width:calc(100vw - 2rem);padding:1.2rem}.toast{bottom:1rem;right:1rem;left:1rem;text-align:center}table{font-size:.7rem}th,td{padding:.3rem .4rem}.info-grid{grid-template-columns:1fr}}
@media(prefers-reduced-motion:reduce){*{transition:none!important}}
...
.pwd-wrap{position:relative}.pwd-wrap input{padding-right:36px}.pwd-toggle{position:absolute;right:8px;top:10px;background:none;border:none;cursor:pointer;font-size:1rem;color:var(--dim);padding:2px 4px;line-height:1;user-select:none}.pwd-toggle:hover{color:var(--text)}
...
<div class="pwd-wrap"><input type="password" id="password" name="password" placeholder="Password" autofocus required><button type="button" class="pwd-toggle" onclick="var i=document.getElementById('password');var s=i.type==='password';i.type=s?'text':'password';this.textContent=s?'🙈':'👁'">👁</button></div>
...
<div class="err">%s</div>
<form method="POST" action="">
<div class="pwd-wrap"><input type="password" name="password" placeholder="Password" autofocus required><button type="button" class="pwd-toggle" onclick="var i=this.parentElement.querySelector('input');var s=i.type==='password';i.type=s?'text':'password';this.textContent=s?'🙈':'👁'">👁</button></div>
<button type="submit">Enter Site</button>
</form>
</div>
</body>
