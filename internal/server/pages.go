...
var pwd=s.protected?'<code style="font-family:var(--mono);font-size:.75rem;color:var(--text)">'+esc(s.password)+'</code> <button class="copy-btn" data-pwd="'+esc(s.password)+'" onclick="event.stopPropagation();copyText(this.getAttribute(\'data-pwd\'))" title="'+t("copy")+'">⧉</button> <button class="copy-btn" onclick="event.stopPropagation();shareSite(\''+esc(s.name)+'\',\''+siteUrl(s.url)+'\','+(s.protected?'\''+esc(s.password)+'\'':'null')+')" title="'+t("share")+'">🔗</button>':'<span style="color:var(--dim)">'+t("none")+'</span> <button class="copy-btn" onclick="event.stopPropagation();shareSite(\''+esc(s.name)+'\',\''+siteUrl(s.url)+'\',null)" title="'+t("share")+'">🔗</button>';
...
.modal .modal-actions{display:flex;gap:.5rem;justify-content:flex-end;margin-top:1rem}
.pwd-wrap{position:relative;display:flex;align-items:center}.pwd-wrap input{width:100%;padding-right:38px!important;box-sizing:border-box}.pwd-toggle{position:absolute;right:8px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:5px;color:var(--dim);padding:0;transition:color .15s,background .15s}.pwd-toggle:hover{color:var(--text);background:var(--surface-2)}.pwd-toggle svg{width:16px;height:16px;flex-shrink:0}
@media(max-width:640px){.navbar{padding:0 .5rem;height:48px}.navbar .logo{font-size:.85rem}.navbar .nav-right{gap:.4rem}.stats-grid{grid-template-columns:1fr}.lang-toggle a{padding:2px 5px;font-size:.7rem}.admin-layout{grid-template-columns:1fr!important;min-height:auto!important}.admin-sidebar{position:sticky!important;top:48px;flex-direction:row;overflow-x:auto;overflow-y:hidden;height:auto;border-right:none;border-bottom:1px solid var(--line);padding:0;z-index:99;background:var(--surface);-webkit-overflow-scrolling:touch;scrollbar-width:none}.admin-sidebar::-webkit-scrollbar{display:none}.admin-nav-item{flex:0 0 auto;white-space:nowrap;padding:.55rem .7rem;font-size:.75rem;flex-direction:row;gap:.3rem;border-left:none;border-bottom:2px solid transparent;border-top:2px solid transparent;justify-content:center;align-items:center}.admin-nav-item.active{border-left:none;border-bottom-color:var(--accent);border-top-color:transparent}.admin-nav-item span{display:inline;font-size:.75rem}.admin-nav-item svg{width:14px;height:14px;flex-shrink:0}.admin-main{padding:.75rem!important}.container{padding:0 .75rem;margin-top:1rem}.modal{max-width:calc(100vw - 2rem);padding:1.2rem}.toast{bottom:1rem;right:1rem;left:1rem;text-align:center}table{font-size:.7rem;display:block;overflow-x:auto;-webkit-overflow-scrolling:touch}th,td{padding:.3rem .4rem}.info-grid{grid-template-columns:1fr}.toggle-row{flex-direction:column;align-items:flex-start;gap:.4rem}.toggle-row .toggle-info{width:100%}.setting-input-group{width:100%;justify-content:flex-start}}
@media(prefers-reduced-motion:reduce){*{transition:none!important}}
...
h+='<div class="toggle-row"><div class="toggle-info"><div class="toggle-label">'+t("maxUploadSize")+'</div><div class="toggle-desc">'+t("maxUploadSizeDesc")+'</div></div><div class="setting-input-group"><input type="number" id="max-upload-size" value="'+mus+'" min="1" max="1024"><span class="unit">MB</span><button class="btn btn-promote" onclick="saveUploadSize()">'+t("save")+'</button></div></div>';
h+='<div class="toggle-row"><div class="toggle-info"><div class="toggle-label">'+t("maxSitesPerUser")+'</div><div class="toggle-desc">'+t("maxSitesPerUserDesc")+'</div></div><div class="setting-input-group"><input type="number" id="max-sites-per-user" value="'+mspu+'" min="0" max="10000"><button class="btn btn-promote" onclick="saveMaxSites()">'+t("save")+'</button></div></div>';
