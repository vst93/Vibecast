...
h+='<li class="site-item"><div class="site-head" onclick="toggleDetail('+s.id+')"><div class="info"><div class="name"><span class="status-dot '+dot+'"></span>'+esc(s.name)+' '+badge+' '+vis+'</div><div class="url">'+esc(s.url)+'</div></div><div class="actions"><a class="btn btn-sm btn-outline" href="'+esc(s.url)+'" target="_blank" onclick="event.stopPropagation()">'+t("visit")+'</a><label class="upload-btn" id="upload-btn-'+s.id+'" onclick="event.stopPropagation()">'+t("deployBtn")+'<input type="file" accept=".zip,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.odt,.ods,.odp,.rtf,.txt,.csv,.json,.xml,.html,.htm,.css,.js,.mjs,.svg,.png,.jpg,.jpeg,.gif,.webp,.avif,.ico,.mp4,.webm,.mp3,.ogg,.wav,.flac,.wasm,.woff,.woff2,.ttf,.otf,.eot,.webmanifest" onchange="deploy('+s.id+',this.files[0])"></label><button class="btn btn-sm btn-danger" onclick="event.stopPropagation();delSite('+s.id+')">'+t("delete")+'</button></div></div><div class="site-detail" id="detail-'+s.id+'"><div class="detail-row"><span class="label">'+t("siteDesc")+'</span><span class="value"><input type="text" id="desc-'+s.id+'" value="'+esc(s.description||'')+'" placeholder="'+t("siteDescPh")+'" style="width:100%;padding:5px 8px;border:1px solid var(--line);border-radius:5px;font-size:.8rem;background:var(--ink);color:var(--text)" onchange="saveDescription('+s.id+')"></span></div><div class="detail-row"><span class="label">'+t("accessPassword")+'</span><span class="value">'+pwd+'</span></div><div class="detail-row"><span class="label">'+t("orgOpenLabel")+'</span><span class="value"><label class="checkbox-label" style="display:inline-flex;align-items:center;gap:.4rem"><input type="checkbox" id="org-open-'+s.id+'" '+(s.orgOpen?'checked':'')+' onchange="toggleOrgOpen('+s.id+')"> <span>'+t("orgOpenDesc")+'</span></label></span></div><div class="detail-row"><span class="label">'+t("orgPinnedLabel")+'</span><span class="value"><label class="checkbox-label" style="display:inline-flex;align-items:center;gap:.4rem"><input type="checkbox" id="org-pinned-'+s.id+'" '+(s.orgPinned?'checked':'')+' onchange="toggleOrgPinned('+s.id+')"> <span>'+t("orgPinnedDesc")+'</span></label></span></div><div class="detail-row"><span class="label">'+t("visits")+'</span><span class="value" style="display:flex;gap:1rem;font-family:var(--mono);font-size:.75rem"><span>'+t("visitsToday")+': <b style="color:var(--accent)">'+vs.today+'</b></span><span>'+t("visitsMonth")+': <b style="color:var(--accent)">'+vs.month+'</b></span><span>'+t("visitsTotal")+': <b style="color:var(--accent)">'+vs.total+'</b></span></span></div></div></li>'}
...
var inp=document.getElementById("desc-"+id);if(!inp)return;
api("/sites/"+id,{method:"PUT",body:JSON.stringify({description:inp.value})}).then(function(){toast(t("save"))}).catch(function(e){toast(e.message,"error")})
}
