...
@media(max-width:768px){.dashboard-grid{grid-template-columns:1fr}.sidebar{position:static}.org-empty{grid-template-columns:1fr}.org-empty-panel:first-child{border-right:none;border-bottom:1px solid var(--line)}.org-bound{grid-template-columns:1fr}.org-bound-left{border-right:none;border-bottom:1px solid var(--line)}.modal-lg{max-width:calc(100vw - 2rem)}}
