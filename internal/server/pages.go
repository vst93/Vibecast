...
h+='</div></div></div><div class="org-bound-right"><div class="org-members-header"><h3>'+t("orgMembers")+'</h3><span class="org-members-count" id="org-members-count">—</span></div><div class="list-toolbar"><input type="text" id="org-member-search" placeholder="'+t("search")+'..." onkeydown="if(event.key===\'Enter\')searchOrgMembers()" value="'+esc(orgMemberSearch)+'"></div><div id="org-member-list"></div></div></div>';
el.innerHTML=h;
loadOrgMembers()
}
...
var lh='<div class="lang-toggle"><a id="theme-toggle" class="btn-icon" onclick="toggleTheme()">'+(getTheme()==="dark"?'` + iconSun + `':'` + iconMoon + `')+'</a><a class="'+(lang==="en"?"active":"")+'" onclick="setLang(\'en\')">EN</a><a class="'+(lang==="zh"?"active":"")+'" onclick="setLang(\'zh\')">中文</a></div>';
