...
var txt=t("shareTemplate").replace("{name}",name).replace("{url}",url).replace("{password}",pwd||"");
copyText(txt);
toast(t("shareCopied"));
}
function createSite(){
...
.modal .modal-actions{display:flex;gap:.5rem;justify-content:flex-end;margin-top:1rem}
.pwd-wrap{position:relative;display:flex;align-items:center}.pwd-wrap input{width:100%;padding-right:38px!important;box-sizing:border-box}.pwd-toggle{position:absolute;right:8px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:5px;color:var(--dim);padding:0;transition:color .15s,background .15s}.pwd-toggle:hover{color:var(--text);background:var(--surface-2)}.pwd-toggle svg{width:16px;height:16px;flex-shrink:0}
@media(max-width:640px){.navbar{padding:0 .5rem;height:48px}.navbar .logo{font-size:.85rem}.navbar .nav-right{gap:.4rem}.stats-grid{grid-template-columns:1fr}.lang-toggle a{padding:2px 5px;font-size:.7rem}.admin-layout{grid-template-columns:1fr!important;min-height:auto!important}.admin-sidebar{position:fixed!important;bottom:0;left:0;right:0;top:auto!important;flex-direction:row;overflow:hidden;height:56px!important;border-right:none;border-top:1px solid var(--line);padding:0;z-index:100;background:var(--surface);justify-content:space-around}.admin-nav-item{flex:1;white-space:nowrap;padding:.35rem .15rem;font-size:.62rem;flex-direction:column;gap:.1rem;border-left:none;border-bottom:2px solid transparent;justify-content:center;align-items:center;min-width:0;overflow:hidden}.admin-nav-item.active{border-left:none;border-bottom-color:var(--accent)}.admin-nav-item span{display:inline;font-size:.62rem;overflow:hidden;text-overflow:ellipsis;max-width:100%}.admin-nav-item svg{width:16px;height:16px;flex-shrink:0}.admin-main{padding:.75rem!important;padding-bottom:72px!important}.container{padding:0 .75rem;margin-top:1rem}.modal{max-width:calc(100vw - 2rem);padding:1.2rem}.toast{bottom:64px;right:1rem;left:1rem;text-align:center}table{font-size:.7rem;display:block;overflow-x:auto;-webkit-overflow-scrolling:touch}th,td{padding:.3rem .4rem}.info-grid{grid-template-columns:1fr}}
@media(prefers-reduced-motion:reduce){*{transition:none!important}}
...
h+='<div class="toggle-row"><div class="toggle-info"><div class="toggle-label">'+t("maxUploadSize")+'</div><div class="toggle-desc">'+t("maxUploadSizeDesc")+'</div></div><div style="display:flex;align-items:center;gap:.5rem"><input type="number" id="max-upload-size" value="'+mus+'" min="1" max="1024" style="width:80px;padding:6px 8px;border:1px solid var(--line);border-radius:5px;font-size:.85rem;background:var(--ink);color:var(--text);text-align:center"><span style="font-size:.75rem;color:var(--dim)">MB</span><button class="btn btn-promote" onclick="saveUploadSize()">'+t("save")+'</button></div></div>';
var mspu=s.maxSitesPerUser||30;
h+='<div class="toggle-row"><div class="toggle-info"><div class="toggle-label">'+t("maxSitesPerUser")+'</div><div class="toggle-desc">'+t("maxSitesPerUserDesc")+'</div></div><div style="display:flex;align-items:center;gap:.5rem"><input type="number" id="max-sites-per-user" value="'+mspu+'" min="0" max="10000" style="width:80px;padding:6px 8px;border:1px solid var(--line);border-radius:5px;font-size:.85rem;background:var(--ink);color:var(--text);text-align:center"><button class="btn btn-promote" onclick="saveMaxSites()">'+t("save")+'</button></div></div>';
