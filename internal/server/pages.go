...
function switchSiteTab(tab){siteTab=tab;var tabs=document.querySelectorAll(".site-tab");for(var i=0;i<tabs.length;i++){tabs[i].classList.remove("active")}var el=document.querySelector('.site-tab[onclick*="switchSiteTab(\\\''+tab+'\\\')"]');if(el)el.classList.add("active");if(siteTab==="my"){loadSites()}else{loadOrgPinnedSites()}}
...
if(!sites.length){el.innerHTML='<div class="list-toolbar"><input type="text" id="site-search" placeholder="'+t("searchSitesPh")+'" onkeydown="if(event.key===\\\'Enter\\\')searchOrgPinned()" value="'+esc(orgPinnedSearch)+'"></div><div class="empty">'+t("noOrgPinned")+'</div>'+pg;return}
var h='<div class="list-toolbar"><input type="text" id="site-search" placeholder="'+t("searchSitesPh")+'" onkeydown="if(event.key===\\\'Enter\\\')searchOrgPinned()" value="'+esc(orgPinnedSearch)+'"></div>';
for(var i=0;i<sites.length;i++){var s=sites[i];
h+='<div class="org-pinned-item"><div class="pinned-head"><div class="pinned-name"><span class="badge badge-pinned">'+t("orgPinnedBadge")+'</span>'+esc(s.name)+'</div><div class="pinned-actions"><a class="btn btn-sm btn-outline" href="'+siteUrl(s.url)+'" target="_blank">'+t("visit")+'</a></div></div></div>'}
...
h+='<li class="site-item"><div class="site-head" onclick="toggleDetail('+s.id+')"><div class="info"><div class="name"><span class="status-dot '+dot+'"></span>'+esc(s.name)+' '+badge+' '+vis+'</div><div class="url">~/sites/'+esc(s.slug)+'/</div></div><div class="actions"><a class="btn btn-sm btn-outline" href="'+siteUrl(s.url)+'" target="_blank" onclick="event.stopPropagation()">'+t("visit")+'</a><label class="upload-btn" id="upload-btn-'+s.id+'" onclick="event.stopPropagation()">'+t("deployBtn")+'<input type="file" accept=".zip,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.odt,.ods,.odp,.rtf,.txt,.csv,.json,.xml,.html,.htm,.css,.js,.mjs,.svg,.png,.jpg,.jpeg,.gif,.webp,.avif,.ico,.mp4,.webm,.mp3,.ogg,.wav,.flac,.wasm,.woff,.woff2,.ttf,.otf,.eot,.webmanifest" onchange="deploy('+s.id+',this.files[0])"></label><button class="btn btn-sm btn-danger" onclick="event.stopPropagation();delSite('+s.id+')">'+t("delete")+'</button></div></div><div class="site-detail" id="detail-'+s.id+'"><div class="detail-row"><span class="label">'+t("accessPassword")+'</span><span class="value">'+pwd+'</span></div><div class="detail-row"><span class="label">'+t("orgOpenLabel")+'</span><span class="value"><label class="checkbox-label" style="display:inline-flex;align-items:center;gap:.4rem"><input type="checkbox" id="org-open-'+s.id+'" '+(s.orgOpen?'checked':'')+' onchange="toggleOrgOpen('+s.id+')"> <span>'+t("orgOpenDesc")+'</span></label></span></div><div class="detail-row"><span class="label">'+t("orgPinnedLabel")+'</span><span class="value"><label class="checkbox-label" style="display:inline-flex;align-items:center;gap:.4rem"><input type="checkbox" id="org-pinned-'+s.id+'" '+(s.orgPinned?'checked':'')+' onchange="toggleOrgPinned('+s.id+')"> <span>'+t("orgPinnedDesc")+'</span></label></span></div><div class="detail-row"><span class="label">'+t("visits")+'</span><span class="value" style="display:flex;gap:1rem;font-family:var(--mono);font-size:.75rem"><span>'+t("visitsToday")+': <b style="color:var(--accent)">'+vs.today+'</b></span><span>'+t("visitsMonth")+': <b style="color:var(--accent)">'+vs.month+'</b></span><span>'+t("visitsTotal")+': <b style="color:var(--accent)">'+vs.total+'</b></span></span></div></div></li>'}
...
function createSite(){
api("/sites",{method:"POST",body:JSON.stringify({name:document.getElementById("site-name").value,password:document.getElementById("site-pwd").value,orgOpen:document.getElementById("site-org-open").checked,orgPinned:document.getElementById("site-org-pinned").checked})}).then(function(){document.getElementById("site-name").value="";document.getElementById("site-pwd").value="";toast(t("siteCreated"));loadSites()}).catch(function(e){toast(e.message,"error")})
}
...
var sad=s.siteAccessDomains||"";
h+='<div style="margin-top:.8rem;padding-top:.8rem;border-top:1px solid var(--line)"><div class="toggle-label" style="margin-bottom:.3rem">'+t("siteAccessDomains")+'</div><div class="toggle-desc" style="margin-bottom:.5rem">'+t("siteAccessDomainsDesc")+'</div>';
if(!sad){h+='<div style="background:rgba(248,81,73,.08);border:1px solid var(--danger);border-radius:6px;padding:.5rem .7rem;margin-bottom:.5rem;font-size:.75rem;color:var(--danger)">'+t("siteAccessDomainsWarn")+'</div>'}
h+='<textarea id="site-access-domains" rows="3" style="width:100%;padding:8px" placeholder="sites.example.com&#10;preview.example.com">'+esc(sad)+'</textarea><div style="margin-top:.4rem"><button class="btn btn-promote" onclick="saveSiteAccessDomains()">'+t("save")+'</button></div></div>';
document.getElementById("settings").innerHTML=h}).catch(function(e){toast(e.message,"error")})
}
function toggleReg(){api("/admin/settings").then(function(d){var c=d.data;return api("/admin/settings",{method:"PUT",body:JSON.stringify({openRegistration:!c.openRegistration,allowPublicAccess:c.allowPublicAccess!==false,domainRestriction:c.domainRestriction,allowedDomains:c.allowedDomains||"",maxUploadSize:c.maxUploadSize||50,maxSitesPerUser:c.maxSitesPerUser||30,siteAccessDomains:c.siteAccessDomains||""})}).then(function(){toast(t("settingsSaved"));loadSettings()})}).catch(function(e){toast(e.message,"error")})}
...
function saveSiteAccessDomains(){var v=document.getElementById("site-access-domains").value;api("/admin/settings").then(function(d){return api("/admin/settings",{method:"PUT",body:JSON.stringify({openRegistration:d.data.openRegistration,allowPublicAccess:d.data.allowPublicAccess!==false,domainRestriction:d.data.domainRestriction,allowedDomains:d.data.allowedDomains||"",maxUploadSize:d.data.maxUploadSize||50,maxSitesPerUser:d.data.maxSitesPerUser||30,siteAccessDomains:v})}).then(function(){toast(t("settingsSaved"));loadSettings()})}).catch(function(e){toast(e.message,"error")})}
